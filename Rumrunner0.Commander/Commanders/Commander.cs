...
using Rumrunner0.Commander.Questions;
...
public abstract class Commander : ICommander
{
	...
	public Dictionary<Command.System.Name, CommandOverride> SystemCommandsOverrides
	{
		init
		{
			...
			{
				if (this._systemCommands.SingleOrDefault(c => c.Settings.Name.Equals(key)) is not { } commandToOverride)
				{
				...
				if (commandOverride.UseAliases is { } useAliases)
				{
					...
					{
						commandToOverride.Settings.Aliases = [];
					}
			...
			Command.System.Help(() =>
			{
				...
				(
					...
					$"\t• {
						...
						this._systemCommands!
							.Select(c => c.AvailableNamesUsingFallback(() => this._settings.UseAliases).Join("/"))
	...
	public ICommander Run()
	{
		...
		var commandsWithNames = commands.Select(c =>
		(
			Command: c,
			Names: c.AvailableNamesUsingFallback(() => this._settings.UseAliases)
		)).ToList();

		var questionCorrectAnswers = commandsWithNames.SelectMany(c => c.Names).ToList();
